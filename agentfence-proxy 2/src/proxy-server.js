...
class ProxyServer {
  ...
  /** Read-only snapshot of live tool state + effective-permissions drift, for callers outside handleMessage (e.g. the public demo API). */
   ...
   *
   * evidenceLogOverride lets a caller (the public demo API) route probe
   * evidence to an ephemeral, in-memory log instead of the durable
   * customer audit trail — anonymous demo traffic must never commingle
   * with, or grow, real evidence. Real /mcp clients calling
   * "agentfence/probe" get no override, so their probe runs are still
   * durably logged like any other decision.
   */
  ...
  /** Runs exactly one named probe (by id) against an (optionally ephemeral) evidence log — powers the "try one attack" interactive demo widget. */
